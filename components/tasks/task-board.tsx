...
export function TaskBoard({ userRole, userId }: TaskBoardProps) {
  ...
  const applyFilters = () => {
    let filtered = [...tasks]
    // ... (lógica de filtros existente permanece a mesma) ...
    if (filters.search) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Cenário 1: Reordenando na mesma coluna
    if (isOverATask) {
    ...
    // Cenário 2: Movendo para uma nova coluna
    const isOverAColumn = over.data.current?.type === 'Column';
    const newStatus = isOverAColumn ? over.id as Task['status'] : tasks.find(t => t.id === overId)?.status;
  ...
  if (loading) {
    return (
      <Card><CardContent className="p-6"><div className="text-center">Carregando tarefas...</div></CardContent></Card>
    )
  }
